...
/*   Updated: 2023/04/01 17:55:19 by doriani          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "test.h"

t_result	test_ft_memcpy(void *dest, const void *src, size_t n, size_t ma_size)
{
	void    *r_lib;
    void    *r_ft;
    void    *copy;
	...
	// r_lib contains the return value of memcpy (pointer to s)
	r_lib = memcpy(dest, src, n);
	...
	// r_ft contains the return value of ft_memcpy (pointer to s)
	r_ft = ft_memcpy(dest, src, n);
	// copies the result in copy_ft
	memcpy(copy_ft, r_ft, ma_size);
	i = 0;
	printf("Dest address content after memcpy:    ");
	while (i < ma_size)
		printf("%2hhx ", ((unsigned char *) copy_lib)[i++]);
	i = 0;
	printf("\n");
	printf("Dest address content after ft_memcpy: ");
	while (i < ma_size)
